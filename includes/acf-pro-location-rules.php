...
namespace ucf_people_directory\acf_pro_location_rules;

const taxonomy_name = 'people_group';

add_action( 'acf/location/rule_types', __NAMESPACE__ . '\\people_group_limited' );
add_action( 'acf/location/rule_values/post_taxonomy_limited', __NAMESPACE__ . '\\people_group_limited_values' );
add_action( 'acf/location/rule_match/post_taxonomy_limited', __NAMESPACE__ . '\\people_group_limited_match', 10, 4 );
...
function people_group_limited_match( $match, $rule, $options, $field_group ) {
	$id       = get_the_ID();
	$term_ids = $options[ "post_terms" ][ taxonomy_name ];
